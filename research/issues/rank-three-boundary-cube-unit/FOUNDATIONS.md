# Foundations and the Rank-Three Terminal Theorem
...
### Theorem 3.1 — no rank-three planar Keller map
...
By Corollary 2.3, such a map is three-sheeted. Orevkov's theorem says the
Jacobian of a three-sheeted polynomial map cannot be constant. This contradicts
`J(P,Q) in C*`. ∎
...
is vacuously true after Theorem 3.1, but recording it as a constructive unit-value
theorem would be misleading. No section is produced and no method is supplied
for higher rank.

## 4. Conditional rank-three algebra retained from the predecessor

For the internal calculation, temporarily ignore Theorem 3.1 and assume a finite
locally free rank-three normal `B`-algebra `O` with generic field `L`. Since
