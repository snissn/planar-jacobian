...
The worker candidate used issue-local labels `R3BC-01` through `R3BC-05`;
`R3BC-07` remains packet-local. This serialized integration maps those reviewed
labels dynamically against base `def93d34174cb87a1d59573bcae395a79b635040`
as `CLM-074` through `CLM-078`. Shared claim, graph, queue, and generated
surfaces are changed only by this integration-maintainer run.
...
Canonical verification is performed in the declared `integration-maintainer` context
