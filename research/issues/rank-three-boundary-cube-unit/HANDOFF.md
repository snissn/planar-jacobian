...
base_main: b4545bd9ca395c023b0d452feee29b5e6f77f83e
scientific_candidate: 4c5d1e1bb8042b046af8af9d2fd764a61e2275e8
...
merge_authority: serialized issue #3 integration under repository policy
```
...
This is allocated as `CLM-074` (packet label `R3BC-01`) at `literature_bound` scope. The English primary full
...
## 3. Allocated global claim deltas

Current-main synchronization allocates the reviewed issue-local labels as follows.
...
After independent primary-source verification, serialized integration applies:

1. add `TERM-RANK-THREE-EXCLUSION`, supported by `CLM-074`;
...
5. preserve `CLM-076` as a residue-class decomposition plus restricted
...
## 5. Shared surfaces synchronized

The integration maintainer synchronizes:
...
Generated views are regenerated by the integration maintainer. The permanent
workflow is updated only to run this packet's guarded verifier and retain
validation logs rather than repository-source exports.
...
The reviewed revision includes exact polynomial-division `H`-adic checks with
negative mutations, the strengthened rare-model control, the geometric and
specified-source-open scope corrections, and the guarded canonical suite.
...
This integration remains off `main` until the non-draft PR passes exact-head
validation and current-head review, is merged under repository policy, and the
same execution verifies the refetched critical files and full suite on live
`main`.
