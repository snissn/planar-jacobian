...
unit. The conditional rank-three algebra still yields `CLM-075`–`CLM-077`:
the geometric special-fiber cubic trichotomy, the exact union over
boundary-primitive residue classes and its within-class pencil identity, and
the differential movement limitation. The specified-source-open countermodel
terminal `CLM-078` remains `literature_bound`.
