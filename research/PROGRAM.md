...
The generic unramified-index bridge is false by `CLM-058`. At rank three, `CLM-074` now gives a separate `literature_bound` terminal: field degree three is generic multiplicity three, which Orevkov's audited primary theorem excludes for polynomial Keller maps. `L14` is disposed without constructing a unit-index section. `CLM-075`–`CLM-078` retain conditional rank-three refinements; nothing here treats degree four or higher or establishes `JC_2`.
